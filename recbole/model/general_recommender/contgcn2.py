#######THIS CODE HAS A CONNECTOR FOR ITEMS AND INITIALIZES USERS THROUGH RAND, VAR, UNI
...
class ContGCN2(GeneralRecommender):
    ...
    def __init__(self, config, dataset):
        super(ContGCN2, self).__init__(config, dataset)
        ...
        if self.use_connector:
            self.item_embedding_connector = EmbeddingConnector(
        ...
        if self.use_connector:
            ...
            self.frozen_item_embedding = torch.nn.Embedding.from_pretrained(self.item_embedding_context, freeze=True)
        ...
        if self.emb_selection:
            ...
            self.item_embedding.weight.requires_grad = False #keep item embedding frozen and only use to initialize user embedding
    ...
    def get_ego_embeddings(self):
        ...
        if self.use_connector:
            #user_embeddings = self.user_embedding_connector(self.frozen_user_embedding.weight)
            item_embeddings = self.item_embedding_connector(self.frozen_item_embedding.weight)
        else:
    ...
    def calculate_loss(self, interaction):
        ...
        if self.use_connector:
            #get the embeddings before model propagation
            #u_ego_embeddings = self.user_embedding_connector(self.frozen_item_embedding.weight) 
            all_item_ego_embeddings = self.item_embedding_connector(self.frozen_item_embedding.weight) 
