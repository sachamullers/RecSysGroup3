#!/bin/bash
#SBATCH --job-name=fair_resid
#SBATCH --partition=gpu_a100
#SBATCH --gpus=1
#SBATCH --time=00:45:00
#SBATCH --cpus-per-task=4
#SBATCH --mem=16G
#SBATCH --output=/gpfs/home5/scur1242/RecSysGroup3/logs_connector/fairness_residual_%j.out
#SBATCH --error=/gpfs/home5/scur1242/RecSysGroup3/logs_connector/fairness_residual_%j.err
...
REC_OUT="data_analysis/fairness_eval/recommendations/beauty_residual_connector_top20.csv"
OUT_DIR="data_analysis/fairness_eval/outputs/beauty_residual_connector_b100_k10"
...
python data_analysis/fairness_eval/export_topk_recommendations.py \
  --model-file saved/ContGCN-Jun-26-2026_21-02-02.pth \
  --output "${REC_OUT}" \
  --k 20 \
  --batch-size 128 \
  --external-ids
...
python data_analysis/fairness_eval/fairness_metrics_posthoc.py \
  ...
  --k 10 \
  --tail-buckets 100 \
  --model-name "Residual connector ContGCN" \
  --output-dir "${OUT_DIR}" \
  --no-plots
...
out_dir = Path("data_analysis/fairness_eval/outputs/beauty_residual_connector_b100_k10")
summary = pd.read_csv(out_dir / "brand_metrics_summary_at_10.csv").iloc[0]

print("\nFinal residual connector fairness values:")
