...
Script to conduct evaluation of cold-start item recommendation using a trained RecBole model checkpoint.
    ...
    residual connector:
        removed item = direct LLMInit selected dimensions
                       + alpha * connector(768-dim removed item LLM embedding)
...
def get_sampled_indices_from_model(model, emb_selection):
    ...
    This is needed for the direct LLMInit baseline and residual connector baseline.
    """
...
def get_embedding_connector(model):
    """
    Your model uses embedding_connector, but this fallback makes the script robust
    in case another branch uses item_embedding_connector.
    """
    if hasattr(model, "embedding_connector"):
        return model.embedding_connector

    if hasattr(model, "item_embedding_connector"):
        return model.item_embedding_connector

    raise RuntimeError(
        "Model has no embedding_connector or item_embedding_connector. "
        "Cannot use connector mode."
    )
...
def make_cold_item_embeddings(mode, model, removed_emb, config, device):
    ...
    connector:
        ...
        If use_residual_connector=True:
            direct_selected_dims + residual_connector_alpha * connector_output
    """
    ...
    if mode == "connector":
        connector = get_embedding_connector(model)
        connector_output = connector(removed_emb)

        use_residual_connector = as_bool(config_get(config, "use_residual_connector", False))
        residual_connector_alpha = float(config_get(config, "residual_connector_alpha", 0.1))

        if use_residual_connector:
            sampled_indices = get_sampled_indices_from_model(model, emb_selection).to(device)
            direct_cold = removed_emb[:, sampled_indices]
            return direct_cold + residual_connector_alpha * connector_output
...
def evaluate_one_mode(
    ...
    if len(relevant_by_user) == 0:
        raise RuntimeError(
            ...
            "are not present in the cold RecBole dataset after filtering."
        )
...
def main():
    ...
    parser.add_argument("--checkpoint", required=True, help="Path to trained cold checkpoint .pth")
    parser.add_argument("--cold_dataset", required=True, help="Example: amazon-beauty-cold10core")
    ...
    print("Device:", device)
    print("use_connector:", config_get(config, "use_connector", False))
    print("emb_selection:", config["emb_selection"])
    print("use_residual_connector:", use_residual_connector)
    print("residual_connector_alpha:", residual_connector_alpha)
    ...
    for mode in modes:
        ...
        if mode == "connector" and use_residual_connector:
            print(f"Connector mode uses residual: direct + {residual_connector_alpha} * connector")
        print("=" * 80)
